...
public:
    Bone( std::forward_list<Bone*> children, const std::string name, const vec3& pos, const vec3& scale, const vec3& rotation = vec3({0, 0, 0}), const vec3& joint = vec3({0, 0, 0}), const int64_t color = 0xFFFFFF );
    Bone( const Bone& rhs );
