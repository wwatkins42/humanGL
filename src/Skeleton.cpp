...
Skeleton::Skeleton( std::unordered_map<std::string, Bone*> bones, const std::string& parentBoneId ) : bones(bones), parentBoneId(parentBoneId) {
    ...
    this->parentBone->getModel()->setRotation(vec3({0, -0.8f, 0}));
}
...
Skeleton& Skeleton::operator=( const Skeleton& rhs ) {
    (void)rhs;
    // this->parentBone = rhs.getParentBone();
    // this->parentBoneId = rhs.getParentBoneId();
    // this->bones = rhs.getBones();
    return (*this);
}
...
void    Skeleton::update( void ) {
    ...
    // this->bones["upperArmLeft"]->getModel()->setRotation(vec3({0, 0, (float)std::cos(glfwGetTime()*4)*3.15f+3.15f}));
